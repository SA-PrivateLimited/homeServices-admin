...
export default function AdminUsersManagementScreen({navigation}: any) {
  ...
  useEffect(() => {
    ...
    const checkAdminAccess = async () => {
      ...
      if (!authUser) {
        Alert.alert('Access Denied', 'You must be logged in to access this screen.');
        navigation.goBack();
        return;
      }
  ...
  const handleChangeUserRole = async (userId: string, currentRole: UserRole | undefined, newRole: UserRole) => {
    ...
    const authUser = auth().currentUser;
    if (!authUser) {
      Alert.alert('Access Denied', 'You must be logged in to change user roles.');
      return;
    }

    // Verify admin role from Firestore
    try {
      const adminDoc = await firestore().collection('users').doc(authUser.uid).get();
    ...
    Alert.alert(
      ...
      [
        {text: 'Cancel', style: 'cancel'},
        {
          ...
          onPress: async () => {
            try {
              // Verify admin access one more time
              const adminUser = auth().currentUser;
              if (!adminUser) {
                Alert.alert('Error', 'You must be logged in to change user roles.');
                return;
              }
              ...
              await firestore().collection('users').doc(userId).update({
                ...
                roleChangedBy: currentUser?.id || adminUser.uid,
                roleChangedAt: firestore.FieldValue.serverTimestamp(),
              });

              // Log the role change for audit (optional - don't fail if this fails)
              try {
                await firestore().collection('roleChangeLogs').add({
                  userId,
                  oldRole: currentRole || 'none',
                  newRole,
                  changedBy: currentUser?.id || adminUser.uid,
                  changedByName: currentUser?.name || adminUser.email || 'Unknown',
              ...
              const errorMessage = error?.message || 'Failed to change user role. Please try again.';
              
              // Show more detailed error message
              let userFriendlyMessage = errorMessage;
              if (errorCode === 'permission-denied') {
                userFriendlyMessage = 'Permission denied. Make sure you are logged in as an admin.';
              } else if (errorCode === 'not-found') {
                userFriendlyMessage = 'User not found. Please refresh the list.';
              }
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterContainer}>
          {(['all', 'customer', 'provider', 'admin'] as const).map(filter => (
            <TouchableOpacity
              ...
              onPress={() => setSelectedRoleFilter(filter as UserRole | 'all')}>
              <Text
                style={[
            ...
            <View key={user.id} style={styles.userCard}>
              ...
              <View style={styles.roleActions}>
                ...
                <View style={styles.roleButtons}>
                  {(['customer', 'provider', 'admin'] as UserRole[]).map(role => (
                    <TouchableOpacity
                      key={role}
                      style={[
                        styles.roleButton,
