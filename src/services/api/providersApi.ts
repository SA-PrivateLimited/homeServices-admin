/**
 * Providers API Service (Admin App)
 * Handles all provider operations via backend API
 */
...
export async function getProviders(filters?: ProviderFilters): Promise<Provider[]> {
  try {
    ...
    const response = await apiGet<{data: Provider[]; count: number} | Provider[]>(endpoint);
...
export async function updateProviderApproval(
  ...
  return apiPut<Provider>(`/providers/${providerId}/approval`, {approvalStatus});
}

export const providersApi = {
  getAll: getProviders,
  getById: getProviderById,
  updateApproval: updateProviderApproval,
};
